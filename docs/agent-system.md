...
本文档约定一套可落地的 **自动化角色** 协作模型：谁与谁对接、文档门禁顺序、测试与部署链如何收紧，并给出可映射到 Cursor 规则 / Skill / 系统提示词的角色清单与图示。主要读者是体系中的 **唯一真人**（你）——负责产品方向与技术取舍、验收关键文档与部署裁决。

单仓库的路径、API、CI 命令与环境等若需落地，请另写项目级文档（例如 `docs/agent-project-<项目名>.md`），**不要**把单项目细节持续写回本文件。
