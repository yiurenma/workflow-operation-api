...
> **快速导航：** [导语](#intro) · [协作原则](#principles) · [读前须知](#read-first) · [角色与职责](#registry) · [唯一真人模式](#human-mode) · [编排完整图](#orchestration-full) · [产品成熟度](#maturity) · [纯自动化链](#autochain) · [落地建议](#execution) · [附录](#appendix)
...
本文档定义一套可复用的 **自动化角色协作框架**：在 **唯一真人（你）** 的前提下，为「产品、架构、测试经理、交付、开发、测试执行、运维」等角色划定 **职责边界、输入输出与禁止项**，并约定 **文档门禁**（PRD / 架构 / 测试文档的定稿与批准顺序）、**测试驱动实现** 与 **部署前放行链**（界面/接口测试 → 测试经理 → 运维）。正文包含 **速查注册表**、**按类展开的角色定义卡**、**协作与数据流图示**（含与「纯自动化」对照）、**产品成熟度（P0–P2）** 及 **落地执行条目**；附录给出 **中英文称谓与代号**，便于编写 **Cursor 规则、Agent Skill 或系统提示词** 时引用一致术语。

**适用范围**：本文件为 **与具体仓库解耦的模板**——不绑定技术栈、目录结构、领域名词或环境标识。**项目级映射**（源码树、OpenAPI 或 CI 命令、密钥与流水线配置、环境矩阵等）应写入仓库内的补充文档（例如 `docs/agent-project-<项目名>.md`），由 **交付 / 架构** 随项目演进维护；**勿将单项目路径与配置细节回写本模板**，以免削弱可迁移性。
...
> 「编排与数据流（完整图）」中的部署关系与本表 **部署与测试汇报链** 一致，仅补充角色节点。
...
> 正文统一用下表**角色称呼**，**不**再写「某某 Agent」。附录指引如下。
...
> 下文 **角色与职责** 一节合并 **速查注册表** 与 **定义卡**（表内约定仍为**草稿**级，可按项目收紧）；该节在「纯自动化链」之后、「落地执行建议」之前。
...
> 部署关系与 **协作原则与门禁** 中 **部署与测试汇报链** 一致；原则与操作约定见该节与 [落地建议](#execution)。
...
> 本节合并 **速查注册表**（输入/输出/禁止项矩阵）与 **定义卡**（按管理 / 开发 / 测试 / 维护 展开，可迁规则骨架）。表内约定仍为**草稿**级，可按项目收紧。
