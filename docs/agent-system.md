...
正文统一用下表**角色称呼**，**不**再写「某某 Agent」、也不中英文各写一遍。需要写进规则文件名、代码常量时，见 **附录 · 英文代号（可选）**。
...
| **测试经理** | 写版本化测试文档、收 **界面测试/接口测试** 汇报、**批准** 后 **运维** 才能部署。 |
...
| **在线后端** | 在线/执行路径上的后端。 |
| **数据库** | 迁移、schema、数据策略。 |
| **界面测试** | 跑 UI 侧测试并汇报 **测试经理**。 |
| **接口测试** | 跑 API 契约测试并汇报 **测试经理**。 |
| **编排**（可选） | 从 **交付** 拆出的编排子能力，**默认**并入 **交付**。 |
...
下表列较多，窄屏阅读时可横向滚动，或宽屏 / 导出 PDF 查阅更清晰。
...
| **实现层** | **开发角色** 在迭代内尽量遵循 **TDD**：对新增行为先补充 **自动化测试**（单元/集成/契约，以仓库约定为准），再写实现至测试通过（Red → Green → Refactor）。 |
| **与界面测试 / 接口测试 的分工** | **测试经理** 定「**测什么、验收标准如何写成可执行条目**」；**界面测试 / 接口测试** 在 **有可运行构建后** 负责 **执行、补充自动化、出测试报告**，并与已定稿测试文档对齐，避免重复与漏测。 |
| **部署与测试汇报链** | **界面测试 / 接口测试** → **测试经理**（汇报）→ **批准** → **运维**（部署到任意目标环境）。**禁止** 界面/接口测试或 **架构** 直连触发 **运维** 部署。 |
...
| **↔ 架构** | **输入**：技术红线、**你熟悉并希望沿用的架构/模式**、非功能约束、对「可试新」的边界（哪些模块可探索）。**对话**：审阅主方案与 **备选/探索方案**，**验收**最终 **架构文档**。 | **架构**；定稿后抄送 **交付**。**交付 不替你与架构对话**。 |
| **↔ 测试经理** | **输入**：已定稿 **PRD** 与 **架构文档**（由 **交付** 协调范围与版本）。**验收**：**测试文档**（测试计划、用例/场景矩阵、与需求追溯关系）。**通过后**才允许进入 **实现开发**。 | **测试经理**；定稿后交 **交付** 作为实现与测试执行的契约。 |
...
| 阶段 | 目标 | 典型产出 | 你重点投入 |
|:---|:---|:---|:---|
| **P0 雏形加固** | 当前仓库可稳定演示：管理 API + 已有 UI 可用、文档与本地/测试环境可复现 | README/环境说明、最小自动化测试、静态 UI 与 API 对齐 | 全栈打通、定规范 |
| **P1 成型 MVP** | 明确 Operation vs Online 边界，各有一条「主路径」端到端可用 | 架构一页纸、核心 API 与错误码稳定、基础可观测 | 架构决策 + 关键代码 |
| **P2 可演示可交付** | 对外讲故事：安全/备份/发布流程有说法，演示脚本可重复 | 发布 checklist、测试报告、产品一页纸（给投资人/早期用户） | 管理节奏 + 质量门禁 |
...
仅在需要写 **Cursor Rule / 常量 / 脚本名** 时使用；**正文叙述不必写**。
