...
这份文档做一件事：说明 **多人分工怎么配合**——在只有 **你** 一个真人的前提下，用「产品、架构、测试经理、交付」等 **角色** 来分工；谁和谁说话、先出哪份文档再写代码、测完谁点头才能上线，都写清楚。后面还有表格和图，方便你对照；若要把这些写进 **Cursor 规则、Skill 或提示词**，也可以按角色一条条拆。

这份文件讲的是 **通用做法**，不写某个具体仓库的文件夹路径、接口地址或 CI 命令。那些请写在 **项目自己的补充文档**里（例如 `docs/agent-project-<项目名>.md`），**不要**把单项目细节不断改回这一份。
