...
这份文档说明：**在只有你一个真人的前提下**，怎样用「产品、架构、测试经理、交付」等 **角色** 把事拆开、谁和谁对接、**先定哪些文档再写代码**、测试和上线谁点头。后面有 **一览表**、**每个角色的细表**、**流程图**（还有和「全自动化」的对照）、产品从雏形到可演示的 **阶段（P0–P2）**，以及 **落地时怎么执行**；附录里有 **中英文称呼和代号**，方便你写 **Cursor 规则、Skill 或提示词** 时叫法一致。

这份文件写的是 **通用套路**，不绑定某个仓库的目录、接口地址或 CI 命令。具体到某一个项目时，请另写一份 **项目补充说明**（例如 `docs/agent-project-<项目名>.md`），把路径、命令、环境等写在那里，一般由 **交付** 或 **架构** 跟着项目改；**不要**把单项目的细节不断改回这一份，否则以后换项目就不好复用了。
